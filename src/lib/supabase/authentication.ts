 ...
 * @param keyId - The ID of the API key to monitor
 * @param token - The Supabase JWT from the client session
 * @param ws - The WebSocket instance
 */
export async function verifyDashboardAccess(keyId: string, token: string): Promise<Res> {
    try {
        ...
        if (keyData.user_id !== user.id) {
            console.warn(`[Dashboard verification] Unauthorized access attempt: User ${user.id} tried to access key ${keyData.id}`);
            return {
                success: false,
